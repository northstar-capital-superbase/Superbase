...
1. **Simple over comprehensive.** A folder only exists if multiple current or clearly-foreseeable notes need it. We did not pre-build structure for hypothetical future needs.
...
5. **Write for the reader three years from now.** Nobody who opens a page should have to ask a person what it means. Placeholders exist so structure can ship before content does — see the placeholder convention below.
...
Never delete a placeholder callout without replacing it with real content — an empty section with no callout looks finished when it isn't, which is worse than an obvious gap.
