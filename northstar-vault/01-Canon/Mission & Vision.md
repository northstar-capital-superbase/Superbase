---
type: canon
owner: "Placeholder — Founder/CEO"
created: 2026-07-22
updated: 2026-07-22
tags: [canon, mission]
---

# Mission & Vision

> [!info] Why this page exists
> Mission and vision are the two sentences everything else in this vault should trace back to. Writing them down, once, in a single canonical place, stops them from drifting into a slightly different version in every deck, README, and job posting.

> [!tip] When to use this page
> Reference it verbatim in external materials rather than re-paraphrasing from memory. Revisit it deliberately (not accidentally) when the company's direction materially changes — log that as an entry in [[History & Milestones]].

> [!note] How it connects to the rest of the vault
> [[Roadmap]] should visibly serve this mission; if a roadmap item doesn't, that's worth questioning. [[Values & Principles]] describes *how* we pursue this mission; this page only describes *what* and *why*.

---

## Mission

> [!todo] Placeholder
> One sentence: what problem does Northstar exist to solve, for whom? Owner: Founder/CEO.

## Vision

> [!todo] Placeholder
> One to three sentences: what does the world look like if Northstar fully succeeds? Owner: Founder/CEO.

## What we are not

> [!todo] Placeholder
> Optional but useful: explicitly naming what's out of scope prevents scope creep in [[Product Index|product]] and [[Roadmap]] decisions later. Owner: Founder/CEO.
